...
# Turn on the camera
cap = cv2.VideoCapture(1)  # Use 0 for built-in webcam or adjust as needed for external webcams
...
while True:
    ...
    hands, img = detector.findHands(img)  # Draw the hand landmarks on the image
    if hands:
        ...
        if not timer_started and not result_displayed:  # Only display "Start" if the timer hasn't started and result isn't displayed
            ...
            if hand_over_text:
                ...
                if not timer_started and not result_displayed:
                    ...
                    fingers = detector.fingersUp(hands[0])  # Check the status of fingers for the first hand
